...
public class ChatService {
  ...
  @Transactional
  public ConversationDto open(UUID targetId) {
    UUID me = current.id();
    ...
    var a = me.toString().compareTo(targetId.toString()) < 0
        ? me
        : targetId;
  ...
  public MessageDto send(
    ...
    UUID other =
        c.getUser1().getId().equals(me)
            ? c.getUser2().getId()
            : c.getUser1().getId();

    if (blocks.existsByBlockerIdAndBlockedId(me, other)
        || blocks.existsByBlockerIdAndBlockedId(other, me)) {
      throw new ForbiddenException("Conversation unavailable");
    }
    ...
    notifications.save(
        Notification.builder()
            ...
            .title("New message")
            .body("You have a new message on SoulSync.")
            .build()
    );
  ...
  public void markRead(UUID conversationId) {
    ...
    var page =
        messages.findByConversationIdOrderByCreatedAtDesc(
            conversationId,
            PageRequest.of(0, 100)
        );

    page.forEach(m -> {
      if (!m.getSender().getId().equals(me)
  ...
  private void authorize(Conversation c) {

    UUID me = current.id();

    if (!c.getUser1().getId().equals(me)
        && !c.getUser2().getId().equals(me)) {
  ...
  private ConversationDto dto(
    ...
    String name = profiles
        .findByUserId(other.getId())
        .map(Profile::getDisplayName)
        .orElse("SoulSync member");
