...
public class SafetyController { private final SafetyService service; public record ReportRequest(@NotBlank @Size(max=200)String reason,@Size(max=2000)String details){}
 ...
 @PostMapping("/reports/{targetId}")ResponseEntity<SafetyService.ReportDto>report(@PathVariable UUID targetId,@Valid @RequestBody ReportRequest r){return ResponseEntity.status(HttpStatus.CREATED).body(service.report(targetId,r.reason(),r.details()));}@GetMapping("/profile-viewers")List<SafetyService.ViewDto>viewers(){return service.viewers();}
}
